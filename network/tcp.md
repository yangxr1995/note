...
Nagle算法和DelayACK不能一起使用，因为Nagle算法导致一定会发送一个小报文，而DelayACK会导致延迟回复小报文的ACK.
...
当没有使用快速重传时，出现失序数据段，发送方会等到缺失报文定时器超时才进行重传,所以效率比较低
...
- 测量方式：通过**统计“交付速率（deliveryRate，一段时间内成功交付的数据量与时间的比值）”**，取其最大值来估计（因为交付速率不会超过瓶颈带宽，所以最大交付速率是BtlBw的无偏估计）。


#### 3. RTprop（Round-Trip Propagation Time，往返传播时延）
是网络路径的**物理属性**（由链路长度、信号传播速度等物理因素决定，如光纤的光速传播时延、地理距离带来的时延），表示**“无排队、无丢包”时，数据往返的最小时延**（即RTT的“极小值”，排除了排队等“噪声”的影响）。
- 类比：若把网络链路想象成“水管”，RTprop就是“水管的长度”，反映路径的固有物理延迟。
- 测量方式：通过 **在一段时间窗口内取RTT的最小值** 来估计（此时排队等噪声的影响最小，接近物理传播的最小时延）。


#### 三者关系与意义
- RTT = RTprop + 排队/处理等“噪声时延”：RTT是“实际观测到的往返时间”，包含了RTprop（物理基础）和网络拥塞等带来的额外时延。
- BtlBw与RTprop共同决定“带宽延迟乘积（BDP = BtlBw × RTprop）”：BDP是网络“管道”能容纳的最大数据量，是拥塞控制（如BBR）的核心参考指标（需让“在途数据量”匹配BDP，以最大化吞吐量且最小化时延）。


#### pacing_gain
由于网络链路是动态变化的，所以需要周期重新测量 BtlBw ,RTprop

大致方法是，定期提升发送速率，和定期降低发送速率。测量 BtlBw 和 RTprop，进而知道链路是否有改变。

`pacing_gain` 是**控制发送速率的“增益系数”**，核心作用是动态调整发送端的发包速度。

### 1. 基本定义与公式
BBR 通过估算**瓶颈带宽（`BtlBw`，路径中最小的链路带宽）**和**最小往返时延（`RTprop`，无排队时的物理传播时延）**来优化传输。`pacing_gain` 是控制**发送速率（`pacing_rate`）**的关键参数，公式为：
`pacing_rate = pacing_gain * BtlBw`
（`BtlBw` 是 BBR 估算的“瓶颈带宽”，`pacing_gain` 通过调整这个乘积，控制发包的“节奏”。）

- BtlBw 是上个周期探测到的最大带宽。
- pacing_rate 是探测是真实发包速率，
- pacing_gain 是速率控制系数，若要探测当前链路带宽是否增大则 pacing_gain > 1.0 ，若要探测当前链路带宽是否减少则 pacing_gain < 1.0

### 2. 不同阶段的动态调整（以 BBRv1 为例）
BBR 分为 **启动（StartUp）、排空（Drain）、探测带宽（ProbeBW）、探测 RTT（ProbeRTT）**等状态，每个状态下 `pacing_gain` 取值不同，以适配网络场景：

- **StartUp（启动阶段）**：
  `pacing_gain` 取较大值（如 `2.89`），让发送速率快速提升，目的是**快速探测瓶颈带宽的上限**（类似“猛踩油门”，尽快找到网络能承载的最大带宽）。此时可能会导致网络队列短暂积压（时延上升），但为了快速“探顶”是必要的。

- **Drain（排空阶段）**：
  `pacing_gain` 取小于 1 的值（如 `0.35`），降低发送速率，目的是**排空启动阶段积压的网络队列**（避免时延持续过高，类似“松油门”释放缓存）。

- **ProbeBW（带宽探测阶段）**：
  `pacing_gain` 会**周期性变化**（如交替使用 `1.25`、`0.75`、`1` 等取值）。这样做是为了 **主动探测带宽是否变化**：
  - 当 `pacing_gain > 1`（如 `1.25`）时，发送速率提高。若网络带宽真的变大，就能利用到更大的带宽；若带宽没变化，网络队列会积压（RTT 上升）。
  - 后续用 `pacing_gain < 1`（如 `0.75`）降低速率，排空队列，同时确认“带宽是否真的提升”。
  这种“周期性波动”让 BBR 能持续感知带宽变化，避免“卡”在旧的带宽估算上。

- **ProbeRTT（RTT 探测阶段）**：
  `pacing_gain` 取 `1`，保持稳定发送，目的是**探测最真实的 `RTprop`**（此时要让网络队列尽可能少，才能测到无排队的物理传播时延）。


### 3. 与 `cwnd_gain` 的配合
BBR 还有另一个参数 `cwnd_gain`，用于控制**拥塞窗口（`cwnd`，最多能发送的未确认数据量）**。
- `pacing_gain` 控制“发送速率的节奏”（**多快发**）；
- `cwnd_gain` 控制“未确认数据的总量”（**发多少**）。
两者配合，让 BBR 既能高效传输数据（充分用带宽），又能避免网络队列过度积压（控制时延）。


### 4. 核心作用：平衡带宽与时延
通过动态调整 `pacing_gain`，BBR 实现了“主动探测 + 精准控制”：
- 当网络带宽有提升空间时，用 `pacing_gain > 1` 探索新的带宽上限；
- 当网络队列开始积压（时延上升）时，用 `pacing_gain < 1` 排空队列，降低时延；
最终让 TCP 连接在**高带宽利用率**和**低排队时延**的状态下稳定运行。


（结合图中“gain > 1 so inflight increases”：当 `pacing_gain > 1` 时，发送速率提高，导致“在途数据量（inflight）”增加；若带宽未提升，RTT 会因队列积压而上升，后续需调整 `pacing_gain` 来平衡。）
...
```c
void onAck(packet) {
  rtt = now - packet.sendtime; // 计算当前数据包的往返时间, 此rtt带有噪音
  update_min_filter(RTpropFilter, rtt); // 基于最新的rtt更新 RTprop，进行降噪
  delivered += packet.size; // 计算累计成功交付数据量
  delivered_time = now;
  deliveryRate = (delivered - packet.delivered) / (delivered_time - packet.delivered_time); // 通过当前与上一次ACK之间的数据增量和时间间隔，估算瞬时带宽（交付速率）
  if (deliveryRate > BtlBwFilter.currentMax || !packet.app_limited) // 如果当前交付速率大于当前记录的最大带 或数据包非应用受限
    update_max_filter(BtlBwFilter, deliveryRate); // 更新最大带宽滤波器（BtlBwFilter），用于估计网络的最大可用带宽
  if (app_limited_until > 0) // 如果存在应用受限标记
    app_limited_until = app_limited_until - packet.size; // 则减少剩余受限数据量
}
```
...
// 1. 计算目标带宽延迟积（BDP）
  // - `BtlBwFilter.currentMax`：当前估计的瓶颈带宽（BtlBw）最大值。
  // - `RTpropFilter.currentMin`：当前估计的最小往返传播时间（RTprop）。
  // - `bdp`（带宽延迟积）：表示网络管道中可容纳的最大数据量（单位：字节）。
  bdp = BtlBwFilter.currentMax × RTpropFilter.currentMin

// 2. 检查是否达到拥塞窗口限制
  // - `inflight`：当前已发送但未确认的数据量。
  // - `cwnd_gain`：拥塞窗口增益系数（通常为2.0，用于容忍突发流量和延迟波动）。
  // - 如果当前在途数据量超过增益后的带宽延迟积（即 `cwnd_gain × bdp`），则暂停发送，等待ACK或超时。
  if (inflight >= cwnd_gain × bdp)
    // wait for ack or retransmission timeout
    return
// 3. 检查是否到达下一个发送时间（ pacing 控制）
  // - `now`：当前时间戳。
  // - `nextSendTime`：根据 pacing 速率计算的下一个数据包发送时间。
  // - 确保数据包按平滑速率（pacing）发送，避免突发流量。
  if (now >= nextSendTime)
// 4. 获取待发送数据包
    packet = nextPacketToSend()
    if (!packet)
      app_limited_until = inflight // 如果没有待发送的数据包，标记应用受限（app-limited）状态（`app_limited_until` 记录当前在途数据量）
      return
// 5. 标记数据包并发送
  // - 记录数据包的应用受限状态、发送时间、已交付数据量（用于后续计算吞吐量）和已交付时间。
  // - `ship(packet)`：实际发送数据包。
